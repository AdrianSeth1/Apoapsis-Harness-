# Comparing a direct frontier task with Apoapsis Harness
...
Give the shared task to the frontier coding tool without Apoapsis. Record:
...
Keep the model/provider and pricing identical to the Apoapsis run. Do not give the
direct run extra repository context unless it is counted as transmitted input.

## Apoapsis run

Install Apoapsis Harness, initialize metadata, and edit `.apoapsis/config.toml`:
...
```bash
apoapsis run "Add resumable downloads.
Preserve the current public API.
Do not add runtime dependencies.
Existing clients must continue working." --context-profile 32k
```

Apoapsis prints the extracted specification and pauses for approval. After approval,
...
```bash
apoapsis inspect TASK-XXXXXXXXXXXX
```
...
| Measure | Direct frontier | Apoapsis local-only | Apoapsis local + frontier |
| --- | ---: | ---: | ---: |
