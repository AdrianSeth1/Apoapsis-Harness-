...
| Item | Current value |
| --- | --- |
| ADR 0091 live Crisis Atlas runner | **Operator preflights refused before inference; remediation in progress, no live arm consumed, 2026-08-01.** The v8 rehearsal and lock remain unchanged and authorize only preflight. The first launch found controller-container wiring defects: the local fake-provider route resolved to Docker Desktop's host gateway; private controller `/tmp` was invisible to the host daemon resolving sibling-workcell mounts; and the resulting workspace was not owned by UID 65532. A second zero-model diagnostic proved the intended 22/22 containment boundary but found an overlong Unix-socket path and controller-private manual mount sources. A third reached the corrected stage-1 topology, then refused because the public session factory did not expose the lower-level builder's host-visible task/forwarder arguments. The remediation now uses loopback, a short host-mounted runtime root, host-visible copies passed through the public factory, and explicit workspace ownership. All attempts exited before `llama-server`, GPU allocation, readiness, or any arm. It must be fully verified, rebuilt, rebound, and pass the same zero-model diagnostic before retry. Details: `docs/evaluation/slice-7p4-live-pilot-runner-2026-08-01.md`. |
