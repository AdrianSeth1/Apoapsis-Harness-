...
| Item | Current value |
| --- | --- |
| ADR 0098 zero-session Capability Sandbox retry | **Implemented; focused verification pending, 2026-08-01.** The real `SLICE-001` retry exposed a second pre-model defect: the Linux launcher required the task worktree's `.git` metadata to be a directory, while attached Git worktrees correctly use a `.git` file. It now uses Git's own worktree predicate and emits explicit preflight diagnostics. Human Review removes `local_continuation` when no local session artifact exists and offers a fresh local run only for an unchanged managed worktree; execution rechecks its fingerprint, removes only that pristine worktree/branch without force, and re-enters the normal approved execution operation. No compatibility fallback or model authority is added. |
