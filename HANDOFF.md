...
| Item | Current value |
| --- | --- |
| ADR 0091 live Crisis Atlas runner | **Third operator attempt reached server launch and refused on an unloadable controller ABI; remediation in progress, no live arm consumed, 2026-08-01.** The corrected zero-model preflight passed 26/26 tool identity and 22/22 containment in the actual container topology. The first cold server then exited 127 before health/readiness: the Debian 12 slim controller lacked `libgomp.so.1`, required glibc 2.38 and GLIBCXX 3.4.32 that its base could not provide, and could not see the host CUDA runtime. There was no readiness request or arm result and cleanup left no server/GPU process. The direct correction moves the controller to digest-pinned Ubuntu 24.04, installs `libgomp1`, mounts the host CUDA runtime read-only, sets the exact server/CUDA loader path, and makes unresolved `ldd` output a pre-start refusal. At the owner's explicit direction, focused/full test suites are skipped for this correction; only compile/diff, image linkage, and zero-token startup checks are authorized before rebuilding and rebinding. Details: `docs/evaluation/slice-7p4-live-pilot-runner-2026-08-01.md`. |
