...
| Item | Current value |
| --- | --- |
| ADR 0091 live Crisis Atlas runner | **Remediated, rebound, and zero-model live preflight passed; ready for a fresh operator retry, no live arm consumed, 2026-08-01.** The first operator attempt and two scripted-only diagnostics exposed and retained four controller-container path/topology defects before inference: controller-local provider addressing, daemon-invisible scratch/mount sources, Unix-socket path length, and a public factory that did not pass through host-visible task/forwarder paths. Runner commit `ec1c934` now uses controller loopback, a separate short host-mounted runtime root, host-visible copies, explicit UID 65532 ownership, and binds the corrected session factory alongside the runner and slot driver. The immutable image is `sha256:b9b1d440fb50...`. The final scripted-only diagnostic passed 26/26 tool identity with exact schema and behavioral read/write/shell proof, plus 22/22 containment, correct mounts, and real egress refusal; its deliberately empty schedule then refused at 0/6 slots. The canonical native-ext4 suite passed **1,948 tests with 13 skips and zero failures** in 348.880 seconds. No `llama-server`, GPU allocation, readiness request, or arm occurred. Details: `docs/evaluation/slice-7p4-live-pilot-runner-2026-08-01.md`. |
