...
| Item | Current value |
| --- | --- |
| ADR 0091 live Crisis Atlas runner | **First operator preflight refused before inference; remediation in progress, no live arm consumed, 2026-08-01.** The v8 rehearsal and lock remain unchanged and authorize only preflight. The first launch rehashed the runtime successfully, then found controller-container wiring defects: hostname resolution sent the controller-local fake-provider route to Docker Desktop's host gateway; preflight scratch lived in the controller's private `/tmp`, invisible to the host daemon that resolves sibling-workcell mounts; and the resulting host-created workspace was not owned by UID 65532. The CLI surface was therefore unobserved and the capability-preserving writable-workspace probe correctly failed. The controller exited, both workcells were removed, `llama-server` never started, GPU VRAM did not change, and provider transcript/arm evidence remained empty. The remediation uses loopback for controller-local upstreams, puts scratch under the identically mounted evidence root, explicitly owns the workspace for the pinned user, and moves the default evidence root out of the root-owned rehearsal directory. It must be fully verified, rebuilt, and rebound before retry. Details: `docs/evaluation/slice-7p4-live-pilot-runner-2026-08-01.md`. |
