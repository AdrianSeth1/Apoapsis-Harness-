...
| Maintenance pass (2026-07-20, external review) | Six fixes from an external code review, verified only under a Linux Python 3.10 sandbox with `StrEnum`/`tomllib`/`datetime.UTC` compatibility shims and no Docker CLI (not yet re-run on the project's own Python 3.12+ Windows environment -- re-run the full suite there before relying on this row). (1) `tests/test_docker_backend.py`/`test_doctor.py` docker fakes now also fake `shutil.which` for the `docker` name only, removing a hidden host dependency that made 19 "deterministic" tests fail on any machine without a Docker CLI -- all pass without Docker now. (2) ADR 0029's two deferred defects are fixed: `RepositoryInspector.search` gains a bounded pure-Python lexical fallback when ripgrep cannot launch (new deterministic test), and `BoundedAgentSession` labels compile-time excerpts of session-patched files `[STALE: ...]` in transmitted context only (new assertions in the existing end-to-end agent test; audit ledger unchanged). (3) Dead `DiscoveryStatus.FRONTIER_PLAN_PROPOSED` value removed (never assigned; `app.js` never rendered it). (4) Version metadata aligned to 1.0 (`pyproject.toml`, `__version__`, research user agent, README). (5) The discovery research chooser now default-checks the effective Auto radio. Under that sandbox, the full suite passed except 6 known interpreter-version artifacts (doctor requires >=3.12; the `download-service-v2` fixture's verification subprocess imports `StrEnum`). |
...
- `src/apoapsis/cli/app.py` owns `apoapsis init`, `ui`, `run`, `task`, `approve`, `inspect`,
  `worktree-create`, `verify`, `rollback`, `frontier-manual` (ADR 0031), and
...
- Read-only task/report/plan/review views, deterministic specification/plan
  ...
  new-task intake (New Task screen, ADR 0023), durable post-approval task
...
- `OPEN_APOAPSIS.cmd` (D5c, ADR 0034; project selection clarified by ADR 0035)
  ...
  non-zero exit on any failure, then runs `apoapsis ui` from the checkout
  (same `PYTHONPATH`-based invocation as Start/Stop, no installed package
  required). It never installs, downloads, or pulls anything, never
  touches Docker/Ollama configuration, and never calls `operator_lifecycle
  stop` -- closing its console window (or Ctrl+C) stops only the one UI
...
### Local-first discovery and frontier planning handoff (ADR 0032)

- `src/apoapsis/discovery/` is a new package sitting in front of Architect
  Mode: a bounded, harness-enforced local-clarification-then-approved-brief
  step, followed by an optional frontier planning stage over either
  transport -- never a general chat.
- `discovery/schema.py` defines `ClarificationQuestion`/
  `ClarificationAnswer`, `IdeaBrief` (`extra="forbid"`, no status field),
  `DiscoveryStatus`/`DiscoverySessionRecord`, and
  `FrontierPlanningRequestPackage`/`FrontierPlanningResponseEnvelope`
  (`extra="forbid"`, a `model_validator` enforcing that exactly one of
  `clarification_questions`/`plan` is ever set).
- `discovery/local_model.py` calls the project's already-configured
  `[models.frontier]` provider (the same model `specification.extractor`
  uses) for two narrow proposals -- clarification questions (harness caps
  the accepted count at `config.discovery.max_clarification_questions`
  regardless of the model's own output count) and an `IdeaBrief` (every
  `key_constraints` entry's `verbatim_source` must be an exact substring of
  the idea text plus the user's own verbatim answers) -- each with exactly
  one bounded correction attempt (ADR 0018's precedent, reused).
...
- `discovery/frontier_package.py` reuses `ContextCompiler`/`GitRepository`
  ...
  configured `[architect.ceilings]`; `package_sha256` follows ADR 0026's
  exclude-the-fresh-identifiers convention.
- `discovery/response.py`'s `apply_frontier_planning_response()` is the one
  place either transport hands a validated response: rejects a stale/
  superseded package or session, enforces
  `config.discovery.max_frontier_clarification_rounds` (default 2) on any
  further clarification-questions response, and -- for a `plan` response --
  calls `SQLitePlanStore.create_plan()` directly, the exact same function
  `architect.importer.import_planner_response()` calls, so the resulting
  plan is an entirely ordinary `PlanRecord` from that point on.
  `architect.schema.PlanRecord.package_id`'s pattern was additively widened
  to accept both `PKG-...` (manual Architect Mode export) and `FPKG-...`
  (this milestone) ids.
- `discovery/manual.py` writes a self-contained
  `FRONTIER-PLANNING-HANDOFF-<package_id>.md` (never automating a
  subscription website) and validates a pasted response (size, UTF-8,
  schema, package-hash self-consistency) before applying it; tokens/cost
  are never recorded for this transport.
- `discovery/api.py` requires `[models.frontier_coder]`; previews the
  configured provider/model and a pessimistic worst-case cost
  (`evaluation.spend_ceiling.estimate_worst_case_call_cost_usd`, reused
  unmodified) before any separate authorization, then wraps the real call
  in `evaluation.spend_ceiling.SpendCeilingModelProvider` (also reused
  unmodified, ADR 0030) so a real call is refused before or after based on
  an explicit `--authorize-planning-spend-usd` ceiling; real measured
  telemetry/cost is always persisted.
...
- `src/apoapsis/manual_frontier/` is a new package, structurally reusing
  (not duplicating) `review/`'s crash-safety ledger: `ReviewOperationStore`,
  `review.execution`'s prepare/run/execute dispatch, and
  `review.classify`'s eligible-action ceiling pattern all handle the actual
  mutation (applying the patch). This package adds only what is genuinely
  new -- the package/response schemas, the immutable handoff-package
  builder, a small preview ledger, the import validator, and the apply
  handler `review.execution` dispatches to.
- `manual_frontier/schema.py` defines `ManualFrontierHandoffPackage`
  (immutable, hashed, bound to task id/version, worktree fingerprint,
  specification/constraints, current diff, relevant evidence/failures, the
  verification catalog, and the exact response JSON schema) and
  `ManualFrontierResponseEnvelope` (`extra="forbid"`: `package_id`/
  `package_sha256`/`task_id`/`task_version`/`patch`/`summary` only -- no
  status, completion, command-selection, or budget field of any kind).
- `manual_frontier/package.py`'s `build_manual_frontier_handoff_package()`
  builds the package from the exact same `ReviewCase` projection every
  other review action uses; `package_sha256` excludes `package_id`/
  `generated_at` (ADR 0026's convention) so re-deriving from the same input
  reproduces the same hash, and `verify_package_integrity()` detects
  on-disk tampering. `write_handoff_artifacts()` writes the canonical JSON
  package plus a single, self-contained
  `FRONTIER-CODING-HANDOFF-<package_id>.md` to the task's existing audit
  directory -- the file a user manually uploads to their own ChatGPT/
  Claude subscription session.
- `manual_frontier/importer.py`'s `import_manual_frontier_response()`
  rechecks task version, eligibility (including the repair-round ceiling),
  worktree fingerprint, active-operation conflicts, package integrity,
  response size (checked in raw bytes before any JSON parsing), schema
  validity, package-hash self-consistency, patch parsing, and patch policy
  -- fails closed on the first violation, otherwise creates an immutable
  `ManualFrontierPreviewRecord` at `PREVIEWED` in its own ledger
  (`.apoapsis/manual-frontier-previews.db`). **Creates a preview only --
  never touches the worktree or task state.**
- Applying the patch requires two explicit steps:
  `manual_frontier/approve.py`'s `approve_manual_frontier_preview()`
  (records intent, rechecks freshness, flips `PREVIEWED -> APPROVED`) and
  then submitting a real `ReviewActionKind.MANUAL_FRONTIER_HANDOFF`
  operation through the unmodified `review.execution.execute_review_action`/
  `prepare_review_operation`/`run_review_operation` seam --
  `ReviewOperationRecord` gained one additive, optional
  `manual_frontier_preview_id` field. `run_review_operation`'s dispatch for
  this action lazily imports `manual_frontier/apply.py`'s
  `execute_manual_frontier_apply()` (avoiding a module-load-time circular
  import, the same technique `ui/server.py` already uses), which
  independently re-checks preview status/task version/fingerprint/package
  integrity again immediately before touching the worktree, applies the
  patch with the same `GitPatchApplier`/`PatchPolicyValidator` every other
  path uses, and runs the same `VerificationRunner` -- **only a passing
  verification result (and, under `STRICT`, satisfied acceptance coverage)
  reaches `COMPLETE`**; nothing in the response envelope can claim it. A
  failing result returns to `HUMAN_REVIEW_REQUIRED` via
  `manual_frontier_apply_verification_failed`, which is both how
  `StopReasonKind.VERIFICATION_FAILED` classification recognizes it and how
  `ReviewCase.manual_frontier_rounds_used` counts one repair round.
- `[manual_frontier]` in `.apoapsis/config.toml` (`max_repair_rounds`,
  default 2; `max_response_bytes`; `max_patch_bytes`) bounds the whole path
  to a small, deterministic number of repair rounds -- never an unbounded
  conversation. `ManualFrontierPreviewRecord.declared_model_name` is
  required, non-empty, operator-typed provenance; Apoapsis never infers or
  verifies which model actually produced a response. No token count or
  cost is ever recorded for this path (there is no telemetry to measure on
  a manual paste), and none is ever displayed as a fabricated `0`.
- CLI: `apoapsis frontier-manual export/import/inspect/approve/apply/status`
...
unlabeled stale/fresh evidence duplication) were resolved on 2026-07-20
(see the Snapshot's maintenance-pass row): `RepositoryInspector.search`
now degrades to a deterministic pure-Python lexical scan when ripgrep
cannot be launched (mirroring the context compiler's own fallback,
labeled `lexical fallback` in the evidence reason), and
`BoundedAgentSession._context_for_turn` now labels any compile-time
excerpt of a file a session patch has since modified with an explicit
`[STALE: ...]` annotation in the transmitted reason -- the audit ledger
copies remain untouched. They were not touched by the D4c runs above.
Full detail:
...
### Browser-product polish and minimal Windows launcher (ADR 0034, D5c, 2026-07-20)

- Decision memo (ADR 0034) compares the existing CLI-plus-system-browser
  surface against a WebView2/pywebview-style native window and a
  Tauri-style wrapper across installation size, signing, updater/security
  surface, toolchain discovery, loopback capability handling, offline
  assets, process ownership/shutdown, model unloading, portability, and
  maintenance complexity. Decision: retain the loopback application plus
  system browser, add one minimal launcher (`OPEN_APOAPSIS.cmd`), and
  defer any native wrapper or installer -- no packaging framework was
  installed and no native window was implemented.
- Full website audit of every existing primary surface (Home, New Task,
  task Specification/Control room/Changes/Human review/Report tabs, Plans,
  Discovery, Evaluations, Models & environment, Human Review queue and
  case detail), not only ADR 0033's new pages. The shipped copy was
  already close to the product-design handoff's calm, direct-language bar;
  this pass removed the one remaining slogan-style headline ("Engineering
  control, without the guesswork." -> "Project status."), renamed the Home
  navigation label from "Projects" to "Home" (a personal-first,
  single-repository product should not imply multiple projects), and added
  `updateDocumentTitle()` so the browser tab title changes with navigation
  (e.g. "Specification · TASK-... — Apoapsis") -- previously the tab title
  never changed from the static `index.html` title regardless of route.
- **Two real bugs found live, not by the deterministic suite** (`apoapsis
  ui` exercised end to end against a disposable initialized project, real
  browser, at 1440px and 1100px):
  1. `changesView()` fell back to the literal string `"baseline"` whenever
     a task had no final report yet, then rendered "The baseline
     completion policy does not gate on acceptance coverage" -- an
     unsupported, specific claim about a policy that could actually be
     `strict` for the project (confirmed live: the QA project's Models &
     environment page correctly showed `Strict`, while the same task's
     Changes tab wrongly implied `baseline`). Fixed by only labeling the
     policy when a report actually states it, and using a neutral,
     report-existence-based message otherwise.
  2. Every detail route (task, plan, plan slice, review case, discovery
     session) left the *previous* record's full content rendered
     underneath a small dismissible-looking error banner when a fetch for
     a new id failed (e.g. a stale link to a deleted or nonexistent task)
     -- a user could easily miss the banner and act on the wrong task's
     specification. Fixed by nulling the relevant store field before each
     refetch, so a failed lookup shows the existing loading/empty state
     (each view already null-guards to `loadingView()`) rather than stale,
     wrong content.
- Accessibility/interaction review found the existing implementation
  already correct on focus-visible outlines, `prefers-reduced-motion`
  gating (transitions only apply under `(prefers-reduced-motion:
  no-preference)`), and the two-step confirmation pattern for destructive
  Human Review actions (`reviewConfirm`/`reviewConfirmPanel`, unchanged);
  no CSS or interaction changes were needed there. Zoom to 150% (via
  `document.documentElement.style.zoom`, since page-zoom keyboard shortcuts
  are not automatable) and keyboard Tab navigation were both checked live
  with no clipping, overlap, or missing focus indication.
- New `tests/test_launcher.py` (10 tests: static content invariants --
  never installs/downloads/reconfigures anything, checks Python/Git/
  initialization in order before launching, points to `STOP_APOAPSIS.cmd`,
  never claims to be a packaged native application -- plus one live
  subprocess test proving the uninitialized-project guard actually exits
  non-zero with the expected message) and new
  `tests/test_ui_copy_and_accessibility.py` (13 tests: a banned-phrase
  scan across every shipped asset, the Home label/hero-copy changes, the
  new `document.title` wiring covering every top-level route, focus-
  visible/reduced-motion CSS invariants, the two-step-confirmation
  regression guard, and regression guards for both live-found bugs above).
  Existing `tests/test_app_js_regression.py` (duplicate-declaration/route-
  dispatch static checks plus the Node syntax/boot smoke tests) continues
  to pass unchanged.
- No hosted call and no live local-model call were made during this pass
  -- every state exercised (specification approval, the control-room
  "Start coding" preview shown then cancelled, changes/review/report empty
  and pending states, Plans/Discovery/Evaluations/Models empty states, an
  invalid-task-id error state) was reachable deterministically via the CLI
  (`apoapsis task`/`apoapsis approve`) or by never confirming a model-
  calling action.
