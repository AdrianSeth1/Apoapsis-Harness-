...
| Item | Current value |
| --- | --- |
| ADR 0091 live Crisis Atlas runner | **Prepared and fully verified deterministically; no live arm consumed, 2026-08-01.** The v8 rehearsal and lock remain unchanged and authorize only preflight. A separate live authorization binds the passed rehearsal, committed live modules, and a provenance-labelled controller image; the WSL launcher additionally requires the operator's literal six-arm acknowledgement and mounts the locked qualification/evaluation inputs read-only. The runner rehashes the GGUF and server closure, refuses a non-cold start, reobserves tool surface and containment with a scripted provider, then runs exactly the frozen schedule with a cold server and one-token readiness request per slot. Control uses Qwen's uninterrupted native loop; sandbox records its first checkpoint and may pass only the controller-produced repair packet through native `--continue`, within the two-continuation ceiling. Raw results stop at `six_slots_complete_pending_independent_scoring`. Focused runner tests passed 14/14 and launcher tests passed 6/6; the canonical Linux/ext4/CPython 3.12 suite at launcher commit `1217243` passed **1,943 tests with 13 skips and zero failures** in 343.721 seconds. The bound controller image is `sha256:41f592a7ee68...`. No `llama-server`, GGUF load, readiness request, or live arm occurred. Details: `docs/evaluation/slice-7p4-live-pilot-runner-2026-08-01.md`. |
