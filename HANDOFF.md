...
| Item | Current value |
| --- | --- |
| ADR 0096 imported plan-response transfer recovery | **Implemented and focused-tested in the working tree, 2026-08-01.** Automatic and next-slice plan runs now recover the exact reported `DirtyParentRepositoryError` case when a manual frontier response was saved as a top-level `apoapsis-plan-response…json` file in the project. Recovery requires semantic equality with a schema-valid canonical response already stored in the discovery audit, then appends only that exact filename to `.git/info/exclude`; it never moves, rewrites, deletes, commits, or broadly ignores user files. The same recovery runs at ordinary execution preflight, while altered lookalikes and all unrelated dirty state still fail closed. Focused plan-auto and execution-authorization coverage passed 19/19. Per explicit owner direction, no full suite was run. Compileall and `git diff --check` passed. No model, provider, container, or network was used. |
