...
| Item | Current value |
| --- | --- |
| ADR 0091 live Crisis Atlas runner | **Server ABI remediated and rebound; ready for a fresh v3 operator retry, no live arm consumed, 2026-08-01.** The corrected zero-model preflight passed 26/26 tool identity and 22/22 containment, but the first cold server exited 127 before health/readiness because the Debian 12 controller could not load the Ubuntu 24.04 server ABI or host CUDA runtime. Runner commit `603e68f` and image `sha256:ddc7e09ce5fc...` use digest-pinned Ubuntu 24.04, install `libgomp1`, mount host CUDA read-only, set only the server/CUDA loader directories, and refuse unresolved `ldd` output before startup. Observed inside that exact image with `--gpus all`: all 22 linkage lines resolved, server version `10107 (c0bc8591e)` executed without loading a model, and the bound runtime rehash/GPU/loader check passed for RTX 4090/24,564 MiB/driver 610.74. At the owner's explicit direction, focused/full suites were skipped for this correction; compileall and diff check passed. No readiness request or arm occurred. Details: `docs/evaluation/slice-7p4-live-pilot-runner-2026-08-01.md`. |
