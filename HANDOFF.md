...
| Item | Current value |
| --- | --- |
| ADR 0091 live Crisis Atlas runner | **V3 reached and completed the first control proposal, then the evaluator refused on Git ownership; minimal v4 correction in progress, 2026-08-01.** Preflight passed, the server loaded, readiness passed, and `crisis-atlas-rep-1-default_qwen_control` completed with 21 model requests, 201,429 input tokens, 2,834 output tokens, 13 tool calls, and a retained worktree. The independent checkpoint then failed because root in the ephemeral Ubuntu controller had not marked the exact owner-mounted seed `.git` as safe. The retained worktree was checkpointed offline after applying that exact trust entry and scored `COMPLETE` with all three criteria satisfied. At the owner's direction there is no resume machinery: v3 remains an aborted diagnostic arm and v4 reruns the frozen six normally. The only code correction sets `safe.directory` to the exact bound seed `.git`; no wildcard. Focused/full suites remain skipped per owner direction; compile/diff and in-image clone/checkpoint are the required checks before rebind. Details: `docs/evaluation/slice-7p4-live-pilot-runner-2026-08-01.md`. |
