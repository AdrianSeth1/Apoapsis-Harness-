...
from apoapsis.intake.execution import run_intake_operation
...
class IntakeWorker:
    ...
    The queue carries only an ``operation_id`` -- ``run_intake_operation``
    reloads the request text, task id, and expected task version from the
    durable ``IntakeOperationRecord`` and freshly rechecks the task
    immediately before doing anything, so a delay between submission and
    execution (or a queued job surviving a restart) can never act on stale
    in-memory state.

    Submission (``prepare_intake_operation``, called synchronously by the
    HTTP handler before ``submit()``) has already durably recorded the
    ...
    (``intake.recovery.recover_stale_intake_operations``): any operation
    still ``RECORDED`` from before this process started is safe to reclaim
    -- nothing was ever transmitted for it -- and is re-enqueued here. A
    ``RUNNING`` operation stale beyond the recovery module's expiry window
    is instead moved to the terminal, inspectable ``AMBIGUOUS`` status and
    ...
    def _recover_at_startup(self) -> None:
        try:
            ...
            operation_store = IntakeOperationStore(
                self.project_root / ".apoapsis" / "intake-operations.db"
            )
        except TaskStoreError:
            return  # a brand-new project has no task database yet
        report = recover_stale_intake_operations(task_store, operation_store)
    ...
    def _run(self) -> None:
        while True:
            ...
            try:
                ...
                # `run_intake_operation` already records failure on the
