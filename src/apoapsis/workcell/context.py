...
#: Patterns that must never appear in the stable prefix. Each one changes
#: between otherwise identical calls and so destroys prefix-cache reuse.
...
class TaskKernel(StrictModel):
    ...
    def validate_stability(self) -> TaskKernel:
        """Refuse anything volatile.

        A kernel containing a timestamp or a run id would change between
        otherwise identical calls, costing every prefix-cache hit for the rest
        of the session while looking like an innocuous detail.
        """
        ...
        for label, pattern in _VOLATILE_PATTERNS:
            ...
            if match:
                raise ValueError(
                    f"the task kernel contains a {label} ({match.group(0)!r}). "
                    "The kernel is part of the cacheable prompt prefix and must "
                    "be identical on every call; put volatile values in the "
                    "observation suffix instead."
                )
        return self
